...
class AccountsController
{
    ...
    public function AgentPayment()
    {
        ...
        $agentDetails = TakeAgentPayment::latest()->get();
        $agents = DB::table('take_agent_payments')
            ->join('agent_details','agent_details.id','=','take_agent_payments.agent_id')
            ->join('visa_stocks','agent_details.id','=','visa_stocks.agent_id')
            //->where('take_agent_payments.agent_id',$agentDetail->id)
            ->get();
        // dd($agents);

        return view('backend.admin.agent-payment.index', compact('agentDetails','agents'));
    }
